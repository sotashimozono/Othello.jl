function Reversi.launch_gui(
    ...
    Label(menu_row[1, 2:3], "BLACK";
          color=_get_color(config, "accent_black"), font=:bold, fontsize=10, halign=:center)
    black_sel = Menu(menu_row[2, 2:3];
        options=menu_options_obs, i_selected=_find_idx(registry_obs[], b_player),
        fontsize=11, width=100, prompt="Human",
        selection_cell_color_inactive=_get_color(config, "accent_black"))

    Label(menu_row[1, 6:7], "WHITE";
          color=_get_color(config, "accent_white"), font=:bold, fontsize=10, halign=:center)
    white_sel = Menu(menu_row[2, 6:7];
    ...
    Label(ctrl[1, 2]; text="Hints",     color=_get_color(config, "text_dim"), fontsize=config.fontsize-1)
    tgl_last  = Toggle(ctrl[1, 3]; active=config.show_last_move)
    Label(ctrl[1, 4]; text="Last Move", color=_get_color(config, "text_dim"), fontsize=config.fontsize-1)
    ...
    kifu_ax = Axis(kifu_panel[2, 1];
        ...
        leftspinevisible=false, rightspinevisible=false,
        topspinevisible=false, bottomspinevisible=false,
        yreversed=true, height=450, valign=:top)
    ...
    # ---------------------------------------------------------------------------
    # Click detection (debug-script Method A: explicit Y-flip)
    # ---------------------------------------------------------------------------
    on(events(fig.scene).mousebutton) do event
        event.button == Mouse.left && event.action == Mouse.press || return
        game          = game_obs[]
        game_over_obs[] && return

        win_pos = events(fig.scene).mouseposition[]
        fig_vp  = fig.scene.viewport[]
        ax_vp   = ax.scene.viewport[]

        # Screen coords have Y=0 at top; GL viewport has Y=0 at bottom → flip
        gl_x = win_pos[1] - fig_vp.origin[1]
        gl_y = fig_vp.widths[2] - win_pos[2] - fig_vp.origin[2]

        # Reject clicks outside the axis viewport
        (0 <= gl_x - ax_vp.origin[1] <= ax_vp.widths[1] &&
         0 <= gl_y - ax_vp.origin[2] <= ax_vp.widths[2]) || return

        norm_x = (gl_x - ax_vp.origin[1]) / ax_vp.widths[1]
        norm_y = (gl_y - ax_vp.origin[2]) / ax_vp.widths[2]
        lims   = ax.finallimits[]
        data_x = lims.origin[1] + norm_x * lims.widths[1]
        data_y = lims.origin[2] + norm_y * lims.widths[2]

        # Must be within board area [0,8] x [0,8]
        (0.0 <= data_x <= 8.0 && 0.0 <= data_y <= 8.0) || return

        col = clamp(Int(floor(data_x)) + 1, 1, 8)
        row = clamp(_BOARD_SIZE - Int(floor(data_y)), 1, 8)
