...
function minimax(game::OthelloGame, depth::Int, maximizing::Bool, alpha::Float64, beta::Float64, player_color::Int)
...
winner_str = winner == BLACK ? "Heuristic (Black)" : (winner == WHITE ? "Random (White)" : "Draw")
...
winner_str = winner == BLACK ? "Minimax (Black)" : (winner == WHITE ? "Random (White)" : "Draw")
println("Winner: $winner_str")
println()
