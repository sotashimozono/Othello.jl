function _open_add_player_dialog!(registry_obs::Observable, update_cb::Function, config::GUIConfig)
    ...
    Label(dlg[1, 1:2]; text="Register Custom Player",
          color=c_text, fontsize=fs+2, font=:bold, halign=:center)
    ...
    btn_cancel = Button(btn_row[1, 1]; label="Cancel",     buttoncolor=c_panel, labelcolor=c_dim,    fontsize=fs)
    btn_reg    = Button(btn_row[1, 2]; label="✔ Register", buttoncolor=c_panel, labelcolor=c_accent, fontsize=fs)
    ...
    on(btn_reg.clicks) do _
        ...
        isempty(raw_name) && (msg_lbl.text[] = "⚠ Please enter a name."; return)
        isempty(raw_expr) && (msg_lbl.text[] = "⚠ Please enter a Julia expression."; return)
