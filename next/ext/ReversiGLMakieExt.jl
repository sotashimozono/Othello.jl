...
using Reversi
using Reversi: BLACK, WHITE, Position, ReversiGame, Player, GUIPlayer, RandomPlayer,
               GameRecord
using Reversi: valid_moves, make_move!, is_game_over, get_winner, count_pieces,
               pass!, get_move, position_to_string
using GLMakie
...
function _draw_board!(ax)
    ...
    for (i, ch) in enumerate('a':'h')
        text!(ax, i - 0.5, 8.18; text=string(ch), color=_C_TEXT, fontsize=15,
              align=(:center,:bottom))
    end
    for r in 1:8
        text!(ax, -0.18, 8 - r + 0.5; text=string(r), color=_C_TEXT, fontsize=15,
              align=(:right,:center))
    end
end

function _draw_pieces!(
    ax, game::ReversiGame, px_per_unit::Real,
    ...
    r  = 0.44
    ms = r * 2 * px_per_unit
    isempty(black_pts) || scatter!(ax, black_pts; color=_C_BLACK_PC, markersize=ms,
                                    strokecolor=RGBf(0.30,0.30,0.32), strokewidth=2.0)
    isempty(white_pts) || scatter!(ax, white_pts; color=_C_WHITE_PC, markersize=ms,
                                    strokecolor=RGBf(0.60,0.60,0.62), strokewidth=2.0)
    if last_move !== nothing
        c0 = last_move.col - 1
        r0 = _BOARD_SIZE - last_move.row
        lines!(ax, [c0, c0+1, c0+1, c0, c0], [r0, r0, r0+1, r0+1, r0];
               color=RGBAf(1.0, 0.35, 0.35, 0.90), linewidth=3.5)
    end
end
...
function _draw_hints!(ax, game::ReversiGame, px_per_unit::Real)
    ...
    return scatter!(ax, pts; color=RGBAf(0.90,0.82,0.10,0.65),
...
function _open_add_player_dialog!(registry_obs::Observable, update_cb::Function)
    ...
    Label(dlg[1,1:3]; text="Register Custom Player", color=_C_TEXT,
    ...
    expr_tb = Textbox(dlg[3,2:3]; placeholder="e.g.  RandomPlayer()", fontsize=13, width=280)

    msg_lbl = Label(dlg[4,1:3]; text="", color=RGBf(1.0,0.4,0.4), fontsize=12, halign=:center)

    btn_reg    = Button(dlg[5,2]; label="✔ Register",
                        buttoncolor=RGBf(0.20,0.45,0.20), labelcolor=:white, fontsize=13)
    btn_cancel = Button(dlg[5,3]; label="Cancel",
                        buttoncolor=RGBf(0.30,0.30,0.34), labelcolor=_C_TEXT_DIM, fontsize=13)

    on(btn_cancel.clicks) do _; close(dlg.scene); end
    ...
    on(btn_reg.clicks) do _
        ...
        isempty(raw_expr) && (msg_lbl.text[] = "⚠ Please enter a Julia expression."; return nothing)
...
function Reversi.launch_gui(
    ...
    _find_idx(reg, p) = begin
        ...
        p isa RandomPlayer && return something(findfirst(e -> e.name == "Random AI", reg), 1)
        return 1
    end

    init_reg         = registry_obs[]
    menu_options_obs = Observable(_names(init_reg))
    on(registry_obs) do reg; menu_options_obs[] = _names(reg); end

    black_sel = Menu(sel_bar[1,2]; options=menu_options_obs,
                    i_selected=_find_idx(init_reg, black), fontsize=13, width=160)
    white_sel = Menu(sel_bar[1,4]; options=menu_options_obs,
                    i_selected=_find_idx(init_reg, white), fontsize=13, width=160)

    Label(sel_bar[1,1]; text="Black:", color=_C_ACCENT_B, fontsize=13, font=:bold,
          halign=:right, tellwidth=false)
    Label(sel_bar[1,3]; text="White:", color=_C_ACCENT_W, fontsize=13, font=:bold,
          halign=:right, tellwidth=false)
    btn_add   = Button(sel_bar[1,5]; label="+ Add Player",
                       buttoncolor=RGBf(0.22,0.35,0.50), labelcolor=:white, fontsize=13)
    btn_start = Button(sel_bar[1,6]; label="▶ New Game",
                       buttoncolor=RGBf(0.22,0.22,0.26), labelcolor=:white, fontsize=13)
    ...
    white_name_lbl  = Label(white_card[1,1]; text="[W]  White: $(_player_name(white))",
                            color=_C_ACCENT_W, fontsize=15, halign=:left, tellwidth=false)
    white_score_lbl = Label(white_card[1,2];
                            text=@lift("$(count_pieces($game_obs)[2])"),
                            color=_C_TEXT, fontsize=20, font=:bold, halign=:right,
                            tellwidth=false)
    white_turn_lbl  = Label(white_card[1,3];
                            text=@lift((!$game_over_obs && $game_obs.current_player==WHITE) ? "< Turn" : ""),
                            color=_C_ACCENT_B, fontsize=13, halign=:left, tellwidth=false)
    rowsize!(fig.layout, 2, Fixed(40))

    # Row 3: board
    ax = Axis(fig[3,1]; aspect=DataAspect(), limits=(-0.40,8.32,-0.24,8.55),
              backgroundcolor=_C_BG, xgridvisible=false, ygridvisible=false,
    ...
    black_name_lbl  = Label(black_card[1,1]; text="[B]  Black: $(_player_name(black))",
                            color=_C_ACCENT_B, fontsize=15, halign=:left, tellwidth=false)
    black_score_lbl = Label(black_card[1,2];
                            text=@lift("$(count_pieces($game_obs)[1])"),
                            color=_C_TEXT, fontsize=20, font=:bold, halign=:right,
                            tellwidth=false)
    black_turn_lbl  = Label(black_card[1,3];
    ...
    status_lbl = Label(ctrl[1,5];
        text=@lift(begin
            game = $game_obs; over = $game_over_obs
            if over
                w = get_winner(game); b, wc = count_pieces(game)
                w==BLACK ? "Black wins!  (B $b - W $wc)" :
                w==WHITE ? "White wins!  (B $b - W $wc)" : "Draw!  (B $b - W $wc)"
            else
    ...
    Label(kifu_panel[1,1]; text="Move History", color=_C_TEXT, fontsize=14,
          font=:bold, halign=:center)
    kifu_ax = Axis(kifu_panel[2,1]; backgroundcolor=_C_PANEL,
    ...
    function _refresh_kifu!(kifu_ax, kifu)
        empty!(kifu_ax)
        if isempty(kifu)
            text!(kifu_ax, 0.5, 0.5; text="No moves yet", color=_C_TEXT_DIM,
                  fontsize=12, align=(:center,:center), space=:relative)
            ylims!(kifu_ax, 1, 0)
            return nothing
        end
        for (n, color, notation) in kifu
            ...
            text!(kifu_ax, 0.05, Float32(n-1); text=lpad(string(n), 3),
                  color=_C_TEXT_DIM, fontsize=12, align=(:left,:top))
            text!(kifu_ax, 0.35, Float32(n-1); text="$pc_tag  $notation",
                  color=line_color, fontsize=12, align=(:left,:top))
        end
        ylims!(kifu_ax, length(kifu) + 0.5, -0.5)
        xlims!(kifu_ax, 0, 1)
    end

    on(game_obs)         do game; _refresh!(ax, game, hints_obs[], last_move_obs[]); end
    on(kifu_obs)         do kifu; _refresh_kifu!(kifu_ax, kifu); end
    on(tgl_hints.active) do v
        hints_obs[] = v; _refresh!(ax, game_obs[], v, last_move_obs[])
    end
    on(tgl_last.active)  do v
        show_last_obs[] = v; _refresh!(ax, game_obs[], hints_obs[], last_move_obs[])
    end
    on(ax.scene.viewport) do _; _refresh!(ax, game_obs[], hints_obs[], last_move_obs[]); end

    players = Ref{Dict{Int,Player}}(Dict(BLACK => black, WHITE => white))

    function _selected_player(menu::Menu)
        reg = registry_obs[]; idx = clamp(menu.i_selected[], 1, length(reg))
    ...
    function start_game!(new_black::Player, new_white::Player)
        ...
        game = ReversiGame(); kifu = Tuple{Int,Int,String}[]
        game_over_obs[] = false; last_move_obs[] = nothing
        kifu_obs[] = kifu; game_obs[] = game
    ...
    on(events(fig.scene).mousebutton) do event
        ...
        game = game_obs[]; game_over_obs[] && return nothing
...
function Reversi.launch_replay_gui(moves::Vector{String}; title::String="Game Replay")
    ...
    # Title bar
    Label(fig[1,1]; text=title, color=_C_TEXT, fontsize=15, font=:bold,
          halign=:left, tellwidth=false)
    Label(fig[1,1];
          text=@lift(begin g=$game_obs; b,w=count_pieces(g); "B $b – W $w" end),
          color=_C_TEXT_DIM, fontsize=14, halign=:right, tellwidth=false)
    rowsize!(fig.layout, 1, Fixed(36))

    # Board
    ax = Axis(fig[2,1]; aspect=DataAspect(), limits=(-0.40,8.32,-0.24,8.55),
              backgroundcolor=_C_BG, xgridvisible=false, ygridvisible=false,
              xticksvisible=false, yticksvisible=false,
              xticklabelsvisible=false, yticklabelsvisible=false,
              leftspinevisible=false, rightspinevisible=false,
    ...
    btn_first = Button(nav[1,1]; label="|◀", buttoncolor=RGBf(0.22,0.22,0.26),
                       labelcolor=:white, fontsize=14, width=44)
    btn_prev  = Button(nav[1,2]; label="◀",  buttoncolor=RGBf(0.22,0.22,0.26),
                       labelcolor=:white, fontsize=14, width=44)
    btn_next  = Button(nav[1,3]; label="▶",  buttoncolor=RGBf(0.22,0.22,0.26),
                       labelcolor=:white, fontsize=14, width=44)
    btn_last  = Button(nav[1,4]; label="▶|", buttoncolor=RGBf(0.22,0.22,0.26),
                       labelcolor=:white, fontsize=14, width=44)
    slider    = Slider(nav[1,5]; range=0:n_moves, startvalue=0)
    Label(nav[1,6]; text=@lift("Move $($pos_obs) / $n_moves"),
          color=_C_TEXT_DIM, fontsize=13, halign=:left, tellwidth=false)
    ...
    function _refresh_board!(p)
        ppu = _px_per_unit_replay(); empty!(ax); _draw_board!(ax)
    ...
    function _refresh_replay_kifu!(current_p)
        empty!(kifu_ax); n = length(moves)
        if n == 0
            text!(kifu_ax, 0.5, 0.5; text="No moves", color=_C_TEXT_DIM, fontsize=12,
                  align=(:center,:center), space=:relative)
            ylims!(kifu_ax, 1, 0); return nothing
        end
        for i in 1:n
            color      = move_colors[i]
            pc_tag     = color == BLACK ? "[B]" : "[W]"
            is_current = (i == current_p)
            row_color  = is_current ? RGBf(1.0,0.85,0.2) :
                         color == BLACK ? _C_ACCENT_B : _C_TEXT
            text!(kifu_ax, 0.05, Float32(i-1); text=lpad(string(i),3),
                  color=_C_TEXT_DIM, fontsize=12, align=(:left,:top))
            text!(kifu_ax, 0.35, Float32(i-1); text="$pc_tag  $(moves[i])",
                  color=row_color, fontsize=is_current ? 13 : 12, align=(:left,:top))
        end
        ylims!(kifu_ax, n + 0.5, -0.5); xlims!(kifu_ax, 0, 1)
    end

    function _goto!(p)
        p = clamp(p, 0, n_moves)
        pos_obs[] = p; game_obs[] = states[p+1]; slider.value[] = p
        _refresh_board!(p); _refresh_replay_kifu!(p)
    end

    on(tgl_last.active)   do v; show_last_obs[] = v; _refresh_board!(pos_obs[]); end
    on(ax.scene.viewport) do _; _refresh_board!(pos_obs[]); end
    ...
    on(slider.value)      do v; v == pos_obs[] && return nothing; _goto!(v); end

    on(events(fig.scene).keyboardbutton) do event
        (event.action == Keyboard.press || event.action == Keyboard.repeat) || return nothing
